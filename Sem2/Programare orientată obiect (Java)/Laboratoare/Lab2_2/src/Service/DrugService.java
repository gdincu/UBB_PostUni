...
public class DrugService extends IsSearchable<Drug> {
    ...
    private Stack<UndoRedo<Drug>> undoableOperations = new Stack<>();
    private Stack<UndoRedo<Drug>> redoableeOperations = new Stack<>();
     ...
     */
    public DrugService(IRepository<Drug> repository, IRepository<Transaction> transactionRepository) {
    ...
    public void addDrug(int id, int price, String name, String producer, boolean receiptNeeded) {
        ...
        if (existing != null) {
            undoableOperations.add(new UpdateOperation<>(repository, existing,drug));
            redoableeOperations.clear();
        } else {
            undoableOperations.add(new AddOperation<>(repository, drug));
            redoableeOperations.clear();
        }
    ...
    public void removeDrug(int id) {
        ...
        Drug temp = repository.findById(id);
    ...
    public void IncreasePrice(int MinPrice, int PctIncrease) {
        for (Drug d : repository.getAll())
            if (d.getPrice() < MinPrice)
                repository.upsert(new Drug(d.getId(),d.getPrice()*(1+PctIncrease/100),d.getName(),d.getProducer(),d.isReceiptNeeded()));
    }
    ...
    public void undo() {
        if (!undoableOperations.empty()) {
            UndoRedo<Drug> lastOperation = undoableOperations.pop();
    ...
    public void redo() {
        if (!redoableeOperations.empty()) {
            UndoRedo<Drug> lastOperation = redoableeOperations.pop();
